...
const run = async () => {
  ...
  const MIN_TWEET_INTERVAL_MS = await getEnvVar(
    'MIN_TWEET_INTERVAL_MS',
    'Enter the minimum tweet interval in milliseconds:'
  )
  const currentTweetInterval = Date.now() - lastTweetedAtMs

  if (currentTweetInterval <= +MIN_TWEET_INTERVAL_MS) {
    l.star(
      `Minimum tweet interval not reached: ${currentTweetInterval}ms elapsed since ${new Date(lastTweetedAtMs).toISOString()}`
    )
    return
  }
  ...
  const tweetHashtagsGeneratorTemplate = loadTemplate(
    'tweet_hashtags_generator.prompt.hbs'
  )

  l.star('Generating tweet...')

  const tweetContent = await callOpenAiApi(
    tweetGeneratorTemplate({
  ...
  const hashtags = await callOpenAiApi(
    tweetHashtagsGeneratorTemplate({
